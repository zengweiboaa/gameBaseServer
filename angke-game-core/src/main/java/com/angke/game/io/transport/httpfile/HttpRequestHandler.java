...
import com.angke.game.io.transport.http.RequestParser;
...
public class HttpRequestHandler extends ChannelInboundHandlerAdapter {
    ...
    static {
        ...
        try {
            String path = location.toURI() + "WebsocketTestClient.html";
    ...
    private boolean cheackUriSuffixIsWebSocketReq(String uri) {
    	boolean result = false;
    	if (uri == null || uri.isEmpty()) {
			return result;
		}
    	if (uri.equalsIgnoreCase(wsUri)) {
    		result = true;
    	}
    	return result;
    }
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
    	if (msg instanceof FullHttpRequest) {
    		...
    		if (cheackUriSuffixIsWebSocketReq(request.uri())) {//ws请求
